...
cargo run -p mail-server --no-default-features --features "sqlite rqlite foundationdb postgres mysql rocks elastic s3 redis" -- --config=/tmp/stalwart-temp-data/config.toml
