...
pub trait SmtpReporting: Sync + Send {
    ...
    fn send_autogenerated(
        &self,
        from_addr: impl Into<CompactString> + Sync + Send,
        rcpts: impl Iterator<Item = impl Into<CompactString> + Sync + Send> + Sync + Send,
        raw_message: Vec<u8>,
...
impl SmtpReporting for Server {
    ...
    async fn sign_message(
        ...
        let signers = self
            .eval_if::<Vec<CompactString>, _>(config, message, message.span_id)
            .await
            .unwrap_or_default();
        if !signers.is_empty() {
