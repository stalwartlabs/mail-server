...
impl TikvStore {
    pub(crate) async fn get_blob(
        ...
        // Check if within keys limit
        if (block_end - block_start) > MAX_SCAN_KEYS_SIZE as usize {
            return Err(trc::Error::new(Store(StoreEvent::BlobRead))
                .reason(format!("blob is larger than maximum amount of keys possible for TiKV scan: {}", MAX_SCAN_KEYS_SIZE)))
        }


        let begin = self
            .new_key_serializer(key.len() + 3, false)
            .write(SUBSPACE_BLOBS)
            .write(key)
            .write(block_start as u16)
            .finalize();
        let key_len = begin.len();
        let begin_range = Bound::Included(TikvKey::from(begin));
        let end = self
            .new_key_serializer(key.len() + 3, false)
            .write(SUBSPACE_BLOBS)
            .write(key)
        ...
        let bound_range = BoundRange::new(begin_range, end_range);
        ...
        'outer: while let Some(key) = keys.next() {
            if key.len() == key_len {
                let value = trx.get(key).await.map_err(into_error)?.unwrap();
    ...
    pub(crate) async fn put_blob(&self, key: &[u8], data: &[u8]) -> trc::Result<()> {
        ...
        // Check if within keys limit
        if last_chunk > MAX_SCAN_KEYS_SIZE as usize {
            return Err(trc::Error::new(Store(StoreEvent::BlobWrite))
                .reason(format!("blob is larger than maximum amount of keys possible for TiKV scan: {}", MAX_SCAN_KEYS_SIZE)))
        }
    ...
    pub(crate) async fn delete_blob(&self, key: &[u8]) -> trc::Result<bool> {
        ...
        let mut keys = trx.scan_keys(
            (
                self.new_key_serializer(key.len() + 3, false)
                    .write(SUBSPACE_BLOBS)
                    .write(key)
                    .write(0u16)
                    .finalize(),
