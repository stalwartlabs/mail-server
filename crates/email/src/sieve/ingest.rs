                        ...
                        if !mailboxes.is_empty() {
                            ...
                            for role in special_use {
                                special_use_ids.push(if role.eq_ignore_ascii_case("inbox") {
                                    ...
                                    if let Ok(role) = SpecialUse::parse_value(&role) {
                                        if let Some(mailbox_id_) = self
                                            .mailbox_get_by_role(account_id, role)
                                            .await
                                            .caused_by(trc::location!())?
                                        {
                            ...
                            for mailbox in mailboxes {
                                match mailbox {
                                    Mailbox::Name(name) => {
                                        if !matches!(
                                            self.mailbox_get_by_name(account_id, &name).await.caused_by(trc::location!())?,
                                            Some(document_id) if special_use_ids.is_empty() ||
